...
def wtp_successful_embed(user: Optional[User], pokemon):
    embed = discord.Embed(
        title=f"{user.display_name if user else 'Alguém'} Acertou!",
        color=0x0099FF
    )
    ...
    embed.set_image(
        url=f"https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/{pokemon['id']}.png"
    )
    embed.set_footer(
        text="PokeBot 2024",
